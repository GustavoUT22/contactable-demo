...
import { createContact, editContact} from "../services/contact-services.js";
import { logout } from "../services/session-services.js";
import { input, selectInput } from "../components/input.js";
import LoginPage from "./login-page.js";
function render() {
    const currectContact = STORE.current_contact
    return `
      <header class="header"><h2>Edit contact</h2>
  ...
  function listenSubmitForm() {
    ...
    form.addEventListener("submit", async (event) => {
      try {
        ...
        const user = await editContact(STORE.current_contact.id, newContactData);
        STORE.user = user;
